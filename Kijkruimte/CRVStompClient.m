...
- (id)initWithHost:(NSString *)theHost 
	...
	if(self = [self initWithHost:theHost
                            port:thePort
                           login:nil
                        passcode:nil
...
- (id)initWithHost:(NSString *)theHost 
		  ...
		  delegate:(id<CRVStompClientDelegate>)theDelegate {
	return [self initWithHost:theHost
                         port:thePort
                        login:theLogin
                     passcode:thePasscode
                        vhost:nil
                     delegate:theDelegate autoconnect:NO];
}
...
- (id)initWithHost:(NSString *)theHost
			 ...
			 login:(NSString *)theLogin
		  passcode:(NSString *)thePasscode
             vhost:(NSString *)theVirtualHost
		  delegate:(id<CRVStompClientDelegate>)theDelegate {
	return [self initWithHost:theHost
                         port:thePort
                        login:theLogin
                     passcode:thePasscode
                        vhost:theVirtualHost
                     delegate:theDelegate
                  autoconnect:NO];
}
...
- (void)connect {
	if(anonymous) {
		...
		NSMutableDictionary *headers = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                 ...
                                 nil];
        if([self vhost])
            [headers setObject:[self vhost] forKey:@"host"];
