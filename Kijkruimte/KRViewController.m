...
NSString *kSCUser = @"HistorischAmsterdam";
...
-(void)viewDidLoad {
    ...
    _currentLocation = [[CLLocation alloc] initWithLatitude:52.376474 longitude:4.900053];
...
-(void)updateTracks:(CLLocation*)location {
    ...
    for(int i = 0; i < [_tracks count]; i++) {
        ...
        if(distance <= DISTANCE && distance > 0.0f) {
            volume = (log(distance/DISTANCE) * -1)/4;
...
-(void)testModeUpdate {
    ...
    double randLat = [self randomDoubleBetween:52.37684 and:52.376303];
    double randLng = [self randomDoubleBetween:4.89875 and:4.899768];
